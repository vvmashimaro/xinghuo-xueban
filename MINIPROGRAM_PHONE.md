...
- 打开小程序，选择"家长/学员"登录
- 输入手机号（任意）和验证码 `888888`，登录
- 进入注册页面 `pages/parent-register/parent-register`
- 填写基础信息（家长称呼、学员昵称、年级等）
- **勾选"我已阅读并同意《个人信息保护政策》"**（必须先勾选）
- 点击"微信授权手机号（推荐）"按钮
- Mock 模式会自动返回 `13980889211`
- 验证页面显示：`已授权手机号: 139****9211` ✅
- 配置学科学情后提交注册
- 成功进入家长主控台

4. **演示步骤 B：预约时授权成功**

- 在家长主控台（`pages/parent-dashboard`）
- 如果账号未绑定手机号，点击任意"预约该导师"按钮
- 系统弹出"预约前需要授权手机号"对话框
- **勾选"我已阅读并同意《个人信息保护政策》"**
- 点击"微信授权手机号（推荐）"
- Mock 模式返回手机号
- 验证显示：`已授权手机号: 138****8000` ✅
- 关闭授权对话框后可继续预约流程

5. **演示步骤 C：授权拒绝 → SMS 备用方案**
...
- 在家长主控台，点击"个人中心"按钮
- 进入 `pages/profile/profile` 页面
- 查看已绑定的脱敏手机号：`138****8000` ✅
- 显示绑定时间
...
- 弹出确认对话框：
  ```
  解绑后您将无法接收课程通知，但账号信息不会被删除。
  当前绑定手机号：139****9211
  ```
- 点击"确认解绑"
- 后端调用 `POST /api/auth/phone/unbind`
- 审计日志记录 `action: unbind`
- 显示"手机号已解绑" ✅
- 手机号状态变为"未绑定"
...
- 弹出警告对话框：
  ...
  ```
- 点击"确认注销"
- 后端调用 `POST /api/auth/phone/cancel`
- 审计日志记录 `action: cancel`
- 用户数据被永久删除
- 显示"账号已注销，正在退出..." ✅
- 自动返回登录页
